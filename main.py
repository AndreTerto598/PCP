...
# Rota para a página de Pedidos a serem entregues

@app.route('/Pedidos_entrega')
def Pedidos_entrega():
    # Buscar todos os pedidos com status "A ser entregue"
    pedidos = PedidoCliente.query.filter_by(status="A ser entregue").all()
    ...
    for pedido in pedidos:
        pedido_formatado = {
            ...
            'data_emissao': pedido.data_emissao.strftime('%d/%m/%Y'),
            'data_entrega': pedido.data_entrega.strftime('%d/%m/%Y'),
            'quantidade': pedido.quantidade,
            'status': pedido.status,
            'rota_selecionada': pedido.rota  # Verifica se a rota já foi definida
        }
...
# Rota para atualizar a rota do pedido
@app.route('/atualizar_rota/<int:pedido_id>', methods=['POST'])
def atualizar_rota(pedido_id):
    pedido = PedidoCliente.query.get_or_404(pedido_id)
    nova_rota = request.form.get('rota')
